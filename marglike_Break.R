    ...
    k = NULL, type = c("drift", "both"), norm = TRUE, ...) 
{
    ...
    (m.star = t(xrho) %*% xrho)
    (det.mstar = abs(det(m.star)))
    (inv.m.star = ginv(X = m.star))
    if ((-0.01 < det.mstar) && (det.mstar < 0.01)) {
        warning("det.mstar==0!! det.mstar<-0.1 has been assigned instead.")
        det.mstar = runif(1, 0, 0.1)
    }
    print(det.mstar)
